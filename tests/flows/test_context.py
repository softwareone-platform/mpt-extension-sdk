...
def test_context_get_order_id(order_factory):
    order = order_factory()
...
def test_context_get_order_type(order_factory):
    order = order_factory()
    context = Context(order)
...
def test_context_get_product_id(order_factory):
    order = order_factory()
    context = Context(order)
...
def test_from_context(order_factory):
    order = order_factory()
    context = Context(order)
...
def test_context_str(order_factory):
    order = order_factory()
    context = Context(order)

    result = str(context)

    assert f"Context: {order.get('id', None)} {order.get('type', None)}" == result
