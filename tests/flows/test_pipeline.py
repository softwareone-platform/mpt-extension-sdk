...
def test_pipeline_completes(mocker, mpt_client):
...
def test_pipeline_exit_prematurely(mocker, mpt_client):
    ...
    pipeline.run(mpt_client, mocker.Mock(spec=Context))  # act
...
def test_pipeline_exception_default_handler(mocker, mpt_client):
    class TestStep(Step):
        def __call__(self, client, context, next_step):
    ...
    with pytest.raises(Exception, match="exception!"):
        pipeline.run(mpt_client, mocker.Mock(spec=Context))
