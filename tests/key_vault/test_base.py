...
def test_init_key_vault(mock_key_vault_name):
    result = KeyVault(mock_key_vault_name)

    assert result.key_vault_name == mock_key_vault_name
...
def test_get_secret(mocker, mock_key_vault_name, mock_secret_name, mock_key_vault_url):
    ...
    result = key_vault.get_secret(mock_secret_name)

    assert result == mock_secret_value.value
...
def test_set_secret(mocker, mock_key_vault_name, mock_secret_name, mock_key_vault_url):
    ...
    result = key_vault.set_secret(mock_secret_name, mock_secret_value.value)
...
def test_set_secret_http_response_error(
    ...
    result = key_vault.set_secret(mock_secret_name, "mock_secret_value")
