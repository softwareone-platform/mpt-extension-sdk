...
def test_create():
    q = RQLQuery()
    assert q.op == RQLQuery.EXPRESSION
    assert q.children == []
...
def test_create_multiple_kwargs():
    ...
    assert q.children[0].op == RQLQuery.EXPRESSION
    assert q.children[0].children == []
    assert str(q.children[0]) == "eq(id,ID)"
    assert q.children[1].op == RQLQuery.EXPRESSION
    assert q.children[1].children == []
    assert str(q.children[1]) == "in(status,(a,b))"
    assert q.children[2].op == RQLQuery.EXPRESSION
