...
def test_build_context_returns_order_context(
    mocker, logger, runtime_settings, event_factory, order_factory, fake_api_service_factory
):
    mocker.patch(
    ...
    fake_api_service = fake_api_service_factory(fake_service)
    ...
    result = asyncio.run(build_context(event_factory("Order", "ORD-1"), logger, fake_api_service))
...
def test_build_context_returns_agreement_context(
    mocker, logger, runtime_settings, event_factory, agreement_factory, fake_api_service_factory
):
    mocker.patch(
        "mpt_extension_sdk.pipeline.factory.get_runtime_settings",
        autospec=True,
    ...
    result = asyncio.run(
        build_context(event_factory("Agreement", "AGR-1"), logger, fake_api_service)
    )
...
def test_build_ctx_rejects_unsupported_obj_type(
    mocker, logger, runtime_settings, event_factory, fake_api_service_factory
):
    mocker.patch(
        "mpt_extension_sdk.pipeline.factory.get_runtime_settings",
        autospec=True,
    ...
    fake_api_service = fake_api_service_factory()

    with pytest.raises(RuntimeError, match="Unsupported context type: Subscription"):
        asyncio.run(build_context(event_factory("Subscription", "SUB-1"), logger, fake_api_service))
...
def test_build_context_carries_contextvars(
    ...
    with event_context_scope():
        result = asyncio.run(
            build_context(event_factory("Order", "ORD-1"), logger, fake_api_service)
        )
