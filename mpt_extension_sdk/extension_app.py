...
from mpt_extension_sdk.pipeline.context.base import ContextAdapter, ExecutionContext
...
class ExtensionRouter:
    ...
    def _register_route(  # noqa: WPS211
        self,
        *,
        name: str,
        path: str,
        ...
        Raises:
            ValueError: If the name, event, or path is empty or already registered.
        """
        if not name.strip():
    ...
    def _with_prefix(self, prefix: str, route: RouteDefinition) -> RouteDefinition:
        ...
        return RouteDefinition(
            name=route.name,
            path=self._join_paths(prefix, route.path),
            event=route.event,
            condition=route.condition,
            task_based=route.task_based,
...
class ExtensionApp:
    ...
    def build_context(self, context: ExecutionContext) -> ExecutionContext:
        ...
        if isinstance(context, OrderContext) and self.order_context_type is not None:
            ...
            expected_type = self.order_context_type.__name__
            if not isinstance(adapted_context, self.order_context_type):
                raise TypeError(f"order_context_type.from_context must return {expected_type}")
            return cast(ExecutionContext, adapted_context)
        if isinstance(context, AgreementContext) and self.agreement_context_type is not None:
            ...
            expected_type = self.agreement_context_type.__name__
            if not isinstance(adapted_context, self.agreement_context_type):
                raise TypeError(f"agreement_context_type.from_context must return {expected_type}")
            return cast(ExecutionContext, adapted_context)
        return context
